...
import { getAllPagos, reset } from '../../../features/pagos/CEBA/pagoSlice';
...
const Pagos = () => {
    ...
    const { pagos, isLoading, isError, message } = useSelector((state) => state.pagos_ceba);
    ...
    const columns = [
        ...
        },
        {
            ...
            cell: row => (
                <div>
                    <Link
                        to={`/ceba/estudiantes/${row?.estudiante?._id}`}
                    >
                        <Text 
                            fontSize='xs'
                            color='blue.500'
